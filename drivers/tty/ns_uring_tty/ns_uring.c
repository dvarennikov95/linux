                                                     ...
                                                     uint8_t pid, bool ignore_full)
{
    ...
    if (ch->upstream) {
        err =
            ns_uring_create_ring(ch->upstream, up_ring_size, up_entry_size, type, pid, ignore_full);
        if (err)
            goto out_err;
    }
    if (ch->downstream) {
        err = ns_uring_create_ring(ch->downstream, down_ring_size, down_entry_size, type, pid,
                                   ignore_full);
        if (err)
            goto out_err;
    }
                                                       ...
                                                       void *ring_up, void *ring_down)
{
    ...
    if (ch->upstream)
        ch->ignore_full = !!ch->upstream->never_full;
    if (ch->downstream)
...
int ns_uring_send(struct ns_uring_ch *ch, void *msg, unsigned size)
{
    ...
    entry = ns_uring_get_next_entry(send_ring,
                                    ns_uring_real_ring_index(send_ring, send_ring->write_ptr));
    if (!entry)
        return -ENOENT;
